...
const handleApiResponse = <T>(res: any, onSuccess: (data: T) => void, onError: (error: string) => void) => {
  if ('error' in res) {
    onError(res.error.data.message);
  } else {
    onSuccess(res.data);
  }
};
...
const createColumnsAndRows = (data: {}[], printData = true, keysToHide = []) => {
  const visibleKeys: string[] = Object.keys(data[0]).filter(key => !keysToHide.includes(key));
  ...
  const columns = visibleKeys.map((key) => {
      ...
      return {
          ...
          formatter(cell: { getData: () => any; }) {
              ...
              return `<div class="font-medium whitespace-nowrap">${rowData[key]}</div>`;
          },
      };
  });
